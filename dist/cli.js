...
try {
    const result = (0, index_1.analyze)(repoPath, opts);
    if (command === "detail") {
        if (!detailEmail) {
            console.error("Usage: git-authors detail <email>");
            process.exit(1);
        }
        const detail = (0, index_1.getAuthorDetail)(result, detailEmail);
        if (!detail) {
            console.error(`Author not found: ${detailEmail}`);
            process.exit(1);
        }
        console.log(jsonOut ? (0, index_1.formatJSON)(detail) : (0, index_1.formatJSON)(detail));
        process.exit(0);
    }
    if (command === "files") {
        if (jsonOut) {
            console.log((0, index_1.formatJSON)(result.fileOwnership.slice(0, top)));
        }
        else {
            console.log((0, index_1.formatFiles)(result.fileOwnership, top));
        }
        process.exit(0);
    }
    // Summary
    if (jsonOut) {
        ...
        console.log((0, index_1.formatJSON)(output));
    }
    else if (mdOut) {
        console.log((0, index_1.formatMarkdown)(result, top));
    }
    else {
        console.log((0, index_1.formatTable)(result, top));
    }
}
catch (e) {
